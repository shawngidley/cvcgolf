...
exports.handler = async (event) => {
  ...
  try {
    ...
    const espnGolfers = competitors.map(c => {
      ...
      // ESPN includes placeholder entries for future rounds — find today's by index if we know the round,
      // otherwise find the last linescore that has actual hole data (linescores nested array)
      let todayLinescore;
      if (currentRound > 0) {
        todayLinescore = linescores[currentRound - 1];
      } else {
        // Fallback: last linescore that has nested hole data
